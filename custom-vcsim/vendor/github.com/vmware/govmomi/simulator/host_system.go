...
var (
	...
	// HostCustomizationFunc is an optional callback invoked after a host is
...
func (h *HostSystem) configure(ctx *Context, spec types.HostConnectSpec, connected bool) {
	...
	// been changed by HostCustomizationFunc). Use a deterministic hash of the
	// hostname to create per-host variation so every host isn't identical.
	hf := fnv.New32a()
	hf.Write([]byte(h.Name))
	hv := hf.Sum32()

	// Variation factor: 0.55 – 0.75 for memory, 0.25 – 0.45 for CPU
	memFraction := 0.55 + float64(hv%2000)/10000.0      // 0.55–0.75
	cpuFraction := 0.25 + float64((hv>>8)%2000)/10000.0 // 0.25–0.45

	memSizeMB := h.Summary.Hardware.MemorySize / (1024 * 1024)
	totalCpuMHz := int64(h.Summary.Hardware.CpuMhz) * int64(h.Summary.Hardware.NumCpuCores)

	h.Summary.QuickStats.OverallMemoryUsage = int32(float64(memSizeMB) * memFraction)
	h.Summary.QuickStats.OverallCpuUsage = int32(float64(totalCpuMHz) * cpuFraction)
	h.Summary.QuickStats.DistributedCpuFairness = int32(2048 + hv%4096)
	h.Summary.QuickStats.DistributedMemoryFairness = int32(1024 + (hv>>4)%2048)
	h.Summary.QuickStats.Uptime = int32(86400 + hv%2592000) // 1–31 days in seconds

	h.Summary.OverallStatus = "green"
...
func (h *HostSystem) QueryHostConnectionInfo(ctx *Context, req *types.QueryHostConnectionInfo) soap.HasFault {
	body := new(methods.QueryHostConnectionInfoBody)

	info := types.HostConnectInfo{
		Host:             h.Summary,
		ClusterSupported: types.NewBool(true),
	}

	// Populate VM summaries from host's VM list
	if h.Vm != nil {
		for _, vmRef := range h.Vm {
			if vm, ok := ctx.Map.Get(vmRef).(*VirtualMachine); ok {
				info.Vm = append(info.Vm, vm.Summary)
			}
		}
	}

	// Populate datastore connect info from host's datastores
	if h.Datastore != nil {
		for _, dsRef := range h.Datastore {
			if ds, ok := ctx.Map.Get(dsRef).(*Datastore); ok {
				dsInfo := &types.HostDatastoreConnectInfo{
					Summary: ds.Summary,
				}
				info.Datastore = append(info.Datastore, dsInfo)
			}
		}
	}

	body.Res = &types.QueryHostConnectionInfoResponse{
		Returnval: info,
	}
	return body
}
