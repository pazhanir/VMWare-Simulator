...
func (ds *Datastore) RefreshDatastore(*Context, *types.RefreshDatastore) soap.HasFault {
	...
	// 8 TB per host for realistic capacity
	ds.Summary.Capacity = int64(units.ByteSize(units.TB)) * 8 * int64(len(ds.Host))
	// Deterministic utilization: 40-70% used based on datastore name hash
	h := fnv.New32a()
	h.Write([]byte(ds.Name))
	usedPct := 40 + int64(h.Sum32()%31) // 40-70%
	ds.Summary.FreeSpace = ds.Summary.Capacity * (100 - usedPct) / 100
