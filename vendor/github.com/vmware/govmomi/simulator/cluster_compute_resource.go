...
func (c *ClusterComputeResource) update(_ *Context, cfg *types.ClusterConfigInfoEx, cspec *types.ClusterConfigSpecEx) types.BaseMethodFault {
	...
	// Sync the deprecated Configuration field from ConfigurationEx
	c.syncConfiguration()
...
func (c *ClusterComputeResource) GetResourceUsage(ctx *Context, req *types.GetResourceUsage) soap.HasFault {
	body := new(methods.GetResourceUsageBody)
	...
	// Memory: TotalMemory is in bytes, convert to MB
	memCapacityMB := int32(summary.TotalMemory / (1024 * 1024))
	// ~60% memory utilization
	memUsedMB := int32(float64(memCapacityMB) * 0.60)

	// Storage: aggregate from cluster datastores
	var storageCapacityMB, storageUsedMB int64
	for _, dsRef := range c.Datastore {
		if ds, ok := ctx.Map.Get(dsRef).(*Datastore); ok {
			storageCapacityMB += ds.Summary.Capacity / (1024 * 1024)
			storageUsedMB += (ds.Summary.Capacity - ds.Summary.FreeSpace) / (1024 * 1024)
		}
	}

	body.Res = &types.GetResourceUsageResponse{
		Returnval: types.ClusterResourceUsageSummary{
			CpuUsedMHz:        cpuUsed,
			CpuCapacityMHz:    cpuCapacity,
			MemUsedMB:         memUsedMB,
...
func CreateClusterComputeResource(ctx *Context, f *Folder, name string, spec types.ClusterConfigSpecEx) (*ClusterComputeResource, types.BaseMethodFault) {
	...
	config := &types.ClusterConfigInfoEx{
		SystemVMsConfig: &types.ClusterSystemVMsConfigInfo{},
	}
	...
	config.DrsConfig.DefaultVmBehavior = types.DrsBehaviorFullyAutomated

	// Apply the caller-provided spec (DasConfig, DrsConfig, etc.)
	if err := cluster.update(nil, config, &spec); err != nil {
		return nil, err
	}
