...
func NewVirtualMachine(ctx *Context, parent types.ManagedObjectReference, spec *types.VirtualMachineConfigSpec) (*VirtualMachine, types.BaseMethodFault) {
	...
	vm.Config = &types.VirtualMachineConfigInfo{
		...
		VmxStatsCollectionEnabled: types.NewBool(true),
	}
...
func (vm *VirtualMachine) apply(spec *types.VirtualMachineConfigSpec) {
	...
	apply := []struct {
		...
		{spec.Annotation, &vm.Summary.Config.Annotation},
...
func (c *powerVMTask) Run(task *Task) (types.AnyType, types.BaseMethodFault) {
	...
	// Simulate VMware Tools reporting guest info on power-on
	if c.state == types.VirtualMachinePowerStatePoweredOn {
		// Generate deterministic hostname and IP from VM name
		vmName := c.VirtualMachine.Name
		h := fnv.New32a()
		h.Write([]byte(vmName))
		hash := h.Sum32()

		// Generate hostname: lowercase VM name with underscores replaced by hyphens
		hostname := strings.ToLower(strings.ReplaceAll(vmName, "_", "-"))

		// Generate deterministic IP in 10.x.y.z range
		// Use hash to spread across /8 space, avoiding .0 and .255
		octet2 := byte((hash >> 16) & 0xFF)
		octet3 := byte((hash >> 8) & 0xFF)
		octet4 := byte(hash & 0xFF)
		if octet2 == 0 {
			octet2 = 1
		}
		if octet3 == 0 {
		...
		if octet4 == 0 || octet4 == 255 {
			octet4 = byte((hash >> 24) & 0xFE)
			if octet4 == 0 {
				octet4 = 1
			}
		}
		ipAddr := fmt.Sprintf("10.%d.%d.%d", octet2, octet3, octet4)

		// Update guest info
		c.VirtualMachine.Guest.HostName = hostname
		c.VirtualMachine.Guest.IpAddress = ipAddr
		c.VirtualMachine.Guest.GuestState = "running"
		c.VirtualMachine.Guest.ToolsStatus = types.VirtualMachineToolsStatusToolsOk
		c.VirtualMachine.Guest.ToolsVersion = "12544"
		c.VirtualMachine.Guest.ToolsRunningStatus = string(types.VirtualMachineToolsRunningStatusGuestToolsRunning)
		c.VirtualMachine.Guest.GuestId = c.VirtualMachine.Config.GuestId
		c.VirtualMachine.Guest.GuestFullName = "VMware Photon OS (64-bit)"

		// Set IPs on existing NICs
		for i := range c.VirtualMachine.Guest.Net {
			c.VirtualMachine.Guest.Net[i].IpAddress = []string{ipAddr}
			c.VirtualMachine.Guest.Net[i].Connected = true
			c.VirtualMachine.Guest.Net[i].IpConfig = &types.NetIpConfigInfo{
		...
		// Populate guest.disk from the VM's virtual disk devices
		disks := object.VirtualDeviceList(c.VirtualMachine.Config.Hardware.Device).SelectByType((*types.VirtualDisk)(nil))
		var guestDisks []types.GuestDiskInfo
		for i, dev := range disks {
			disk := dev.(*types.VirtualDisk)
			capacityBytes := getDiskSize(disk)
			if capacityBytes == 0 {
				capacityBytes = 50 * 1024 * 1024 * 1024 // default 50 GB
			}
			// Deterministic free space: 30-60% free based on hash + disk index
			freePct := int64(30 + (int64(hash)+int64(i)*7)%31)
			freeSpace := capacityBytes * freePct / 100

			// First disk is C:\, subsequent are D:\, E:\, etc.
			driveLetter := string(rune('C' + i))
			guestDisks = append(guestDisks, types.GuestDiskInfo{
		...
		// If no virtual disks found, add a default C: drive
		if len(guestDisks) == 0 {
			defaultCap := int64(50 * 1024 * 1024 * 1024) // 50 GB
			freePct := int64(30 + hash%31)
			guestDisks = append(guestDisks, types.GuestDiskInfo{
				DiskPath:       "C:\\",
				Capacity:       defaultCap,
				FreeSpace:      defaultCap * freePct / 100,
				FilesystemType: "ext4",
			})
		}
		c.VirtualMachine.Guest.Disk = guestDisks

		// Update summary.guest
		c.VirtualMachine.Summary.Guest = &types.VirtualMachineGuestSummary{
			GuestId:            c.VirtualMachine.Config.GuestId,
		...
		// --- Populate VM summary.quickStats with realistic values ---
		memorySizeMB := c.VirtualMachine.Config.Hardware.MemoryMB
		numCPU := c.VirtualMachine.Config.Hardware.NumCPU

		// Derive per-host CPU MHz from the host this VM is on (default 2100 if unavailable)
		cpuMhz := int32(2100)
		if c.VirtualMachine.Runtime.Host != nil {
			if hostObj := c.ctx.Map.Get(*c.VirtualMachine.Runtime.Host); hostObj != nil {
				if hs, ok := hostObj.(*HostSystem); ok {
					cpuMhz = int32(hs.Summary.Hardware.CpuMhz)
				}
			}
		}
		totalCpuMHz := int32(numCPU) * cpuMhz

		// Use FNV hash for deterministic per-VM variation
		memFraction := 30 + int32(hash%21)      // 30-50%
		hostMemFraction := 35 + int32(hash%21)  // 35-55%
		cpuFraction := 25 + int32((hash>>4)%21) // 25-45%

		guestMemUsage := int32(memorySizeMB) * memFraction / 100
		hostMemUsage := int32(memorySizeMB) * hostMemFraction / 100
		overallCpuUsage := totalCpuMHz * cpuFraction / 100
		overallCpuDemand := overallCpuUsage + int32(hash%50)
		sharedMem := int32(24) // 24 MB typical shared
		privateMem := int32(hostMemUsage) - sharedMem

		// Uptime: 1-30 days in seconds
		uptimeSec := int32(86400 + int32(hash%2506000))

		c.VirtualMachine.Summary.QuickStats = types.VirtualMachineQuickStats{
			OverallCpuUsage:              overallCpuUsage,
			OverallCpuDemand:             overallCpuDemand,
			GuestMemoryUsage:             guestMemUsage,
			HostMemoryUsage:              hostMemUsage,
			GuestHeartbeatStatus:         types.ManagedEntityStatusGreen,
			DistributedCpuEntitlement:    overallCpuUsage,
			DistributedMemoryEntitlement: int32(guestMemUsage),
			StaticCpuEntitlement:         int32(totalCpuMHz),
			StaticMemoryEntitlement:      int32(memorySizeMB),
			GrantedMemory:                hostMemUsage,
		...
		// Set summary.runtime.maxMemoryUsage = configured memory (no limit)
		c.VirtualMachine.Summary.Runtime.MaxMemoryUsage = int32(memorySizeMB)

		changes = append(changes,
			types.PropertyChange{Name: "guest.hostName", Val: hostname},
			types.PropertyChange{Name: "guest.ipAddress", Val: ipAddr},
			types.PropertyChange{Name: "guest.guestState", Val: "running"},
			types.PropertyChange{Name: "guest.toolsStatus", Val: types.VirtualMachineToolsStatusToolsOk},
			types.PropertyChange{Name: "guest.toolsVersion", Val: "12544"},
			types.PropertyChange{Name: "guest.toolsRunningStatus", Val: string(types.VirtualMachineToolsRunningStatusGuestToolsRunning)},
			types.PropertyChange{Name: "guest.guestId", Val: c.VirtualMachine.Config.GuestId},
			types.PropertyChange{Name: "guest.guestFullName", Val: "VMware Photon OS (64-bit)"},
			types.PropertyChange{Name: "guest.net", Val: c.VirtualMachine.Guest.Net},
			types.PropertyChange{Name: "guest.disk", Val: c.VirtualMachine.Guest.Disk},
			types.PropertyChange{Name: "summary.guest", Val: c.VirtualMachine.Summary.Guest},
			types.PropertyChange{Name: "summary.quickStats", Val: c.VirtualMachine.Summary.QuickStats},
			types.PropertyChange{Name: "summary.runtime.maxMemoryUsage", Val: c.VirtualMachine.Summary.Runtime.MaxMemoryUsage},
		)
	} else {
		// Power off / suspend: clear guest running state
		c.VirtualMachine.Guest.GuestState = "notRunning"
		c.VirtualMachine.Guest.ToolsRunningStatus = string(types.VirtualMachineToolsRunningStatusGuestToolsNotRunning)
		changes = append(changes,
			types.PropertyChange{Name: "guest.guestState", Val: "notRunning"},
			types.PropertyChange{Name: "guest.toolsRunningStatus", Val: string(types.VirtualMachineToolsRunningStatusGuestToolsNotRunning)},
		)
	}

	c.ctx.Update(c.VirtualMachine, changes)
