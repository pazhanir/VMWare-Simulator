...
func (s *PrincipalManagementService) createDefaultUser(u *url.Userinfo) {
	// Parse the username the same way validLogin/parseID does — split on "@"
	// so that "administrator@vsphere.local" becomes Name="administrator", Domain="vsphere.local".
	// Without this, the stored key would be {Name: "administrator@vsphere.local", Domain: "vsphere.local"}
	// but validLogin would look up {Name: "administrator", Domain: "vsphere.local"} — a mismatch.
	id := parseID(u.Username())
